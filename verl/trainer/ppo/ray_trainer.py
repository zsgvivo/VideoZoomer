...
class RayPPOTrainer(object):
    ...
    def _maybe_log_tool_call_metrics(self, batch, metrics):
        ...
        for idx, success_tool_call in enumerate(success_tool_call_list):
            if self.config.actor_rollout_ref.rollout.tool_call == 'video_zoom':
                ...
                raise NotImplementedError
            # success_tool_call = len(multi_modal_item['image']) > batch.non_tensor_batch['prompt_img_num'][idx]
            # if success_tool_call:
            #     tool_call_frame_num.append(len(multi_modal_item['image']) - batch.non_tensor_batch['prompt_img_num'][idx])
        ...
        if batch.meta_info.get('validate', False):  # val
            ...
            if len(successful_tool_call_to_log) > 0 and multi_modal_data is not None:
                ...
                for sample in samples:
                    ...
                    # row_data.extend([inputs, outputs, ground_truth, score])
                    # for image in multi_modal_data[successful_tool_call_to_log]['image']:
                    #     row_data.append(wandb.Image(image))
                    while len(row_data) < len(columns):
                        # row_data.append(None)
                        # row_data.append(wandb.Image('/mnt/bn/tiktok-mm-4/aiic/users/dingyang/data/blank_image.png'))
    ...
    def _validate(self):
        ...
        if self.tool_call in ['video_zoom']:
            ...
            # success_tool_call = torch.tensor([len(item['image']) > prompt_img_num for item, prompt_img_num in zip(total_test_batch.non_tensor_batch['multi_modal_data'], total_test_batch.non_tensor_batch['prompt_img_num'])])
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # print('new_batchnon_tensor_batch', new_batch.non_tensor_batch.keys())
                # print('new_batch_tensor_batch', new_batch.batch.keys())
                # import pdb; pdb.set_trace()
                num_gen_batches += 1
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        if self.config.trainer.rejection_sample:
                            ...
                            if num_prompt_in_batch < prompt_bsz:
                                ...
                                # print("e0: type(batch): ", type(batch))
                                # import pdb; pdb.set_trace()

                                batch = batch[:traj_bsz]

                                # print("e1: type(batch): ", type(batch))
                                # import pdb; pdb.set_trace()

                                batch = dataprotoitem_to_dataproto(batch)

                                # print("e2: type(batch): ", type(batch))
                                # import pdb; pdb.set_trace()
                        ...
                        # if self.config.reward_model.log_rewards_separately:
                        #     batch.batch['format_scores'] = self.reward_fn.cal_format_reward_for_logging(batch)
                        #     batch.batch['acc_scores'] = self.reward_fn.cal_acc_reward_for_logging(batch)
