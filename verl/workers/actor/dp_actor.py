...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        multi_modal_inputs = {}
        # print("input", micro_batch['input_ids'].shape)
        # print(len(micro_batch['multi_modal_inputs']))
        if 'multi_modal_inputs' in micro_batch:
            for key in micro_batch['multi_modal_inputs'][0].keys():
                multi_modal_inputs[key] = torch.cat([inputs[key] for inputs in micro_batch['multi_modal_inputs']],
                                                    dim=0)
                # multi_modal_inputs[key] = torch.stack([torch.cat(inputs[key], dim=0) for inputs in micro_batch['multi_modal_inputs']], dim=0) 
        # print(multi_modal_inputs['image_grid_thw'])
    ...
    def update_policy(self, data: DataProto):
        ...
        use_multi_turn_response_mask = 'multi_turn_response_mask' in data.batch.keys()
