...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        if role == 'actor':
            ...
            scheduler_type = optim_config.get('lr_scheduler', None)
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
        ...
        elif self.config.rollout.name.startswith('vllm_video'):
            if self.config.rollout.use_fire_sampling:
            ...
            if vllm_mode == 'customized':
                ...
                if self.config.rollout.tool_call == 'video_zoom':
                    from verl.workers.rollout.vllm_rollout import vLLMAsyncRollout
                    vllm_rollout_cls = vLLMRollout_MultiTurn_Video if self.config.rollout.mode == "sync" else vLLMAsyncRollout                
                    rollout = vllm_rollout_cls(model_path=local_path,
                                        config=self.config.rollout,
                                        tokenizer=self.tokenizer,
                                        ...
                                        model_hf_config=self.actor_model_config,
                                        device_mesh=rollout_device_mesh)
                elif self.config.rollout.tool_call == None:
