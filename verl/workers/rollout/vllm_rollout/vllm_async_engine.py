...
# from verl.trainer.constants import TOOL_CALL_CROP_MULTI_TRUN_PROMPT, TOOL_CALL_MULTI_TRUN_PROMPT, SIMPLE_RESIZE_IMAGE_MULTI_TRUN_PROMPT, SIMPLE_THINK_MULTI_TURN_PROMPT, ERROR_INFO_MULTI_TURN_PROMPT, TOOL_CALL_CROP_PROMPT_MAP
# from .function_tools import prepare_grounding_inputs, prepare_grounding_inputs_multi_turn, prepare_tool_call_inputs, resize_image, crop_image, get_valid_mask
...
#         data_mask = ['aiic' in data_source for data_source in data_source_list]
#         data_mask = torch.tensor(data_mask, dtype=torch.bool)

#         masked_sample_num = data_mask.sum().item()

#         # print("masked_sample_num: ", masked_sample_num)

#         if masked_sample_num == 0:
#             print(f"ready to call multiple-choice reward function for {len(data_source_list)} samples")
#             reward_tensor, acc_reward_tensor, format_reward_tensor, overlong_reward_tensor, invalid_uids = reward_fn(cur_batch)
#         elif masked_sample_num == len(data_source_list):
#             print(f"ready to call gpt reward function for {masked_sample_num} samples (total {len(data_source_list)})")
#             reward_tensor, acc_reward_tensor, format_reward_tensor, overlong_reward_tensor, invalid_uids = gpt_reward_fn(cur_batch)
#         else:
#             normal_batch = dataprotoitem_to_dataproto(cur_batch[~data_mask])
#             gpt_batch = dataprotoitem_to_dataproto(cur_batch[data_mask])
#             reward_tensor_normal, acc_reward_tensor_normal, format_reward_tensor_normal, overlong_reward_tensor_normal, invalid_uids_normal = reward_fn(normal_batch)
#             reward_tensor_gpt, acc_reward_tensor_gpt, format_reward_tensor_gpt, overlong_reward_tensor_gpt, invalid_uids_gpt = gpt_reward_fn(gpt_batch)

#             reward_tensor = torch.zeros((reward_tensor_normal.shape[0] + reward_tensor_gpt.shape[0], reward_tensor_normal.shape[-1]), dtype=reward_tensor_normal.dtype, device=reward_tensor_normal.device)
#             acc_reward_tensor = torch.zeros_like(reward_tensor)
#             format_reward_tensor = torch.zeros_like(reward_tensor)
#             overlong_reward_tensor = torch.zeros_like(reward_tensor)
#             reward_tensor[~data_mask] = reward_tensor_normal
#             reward_tensor[data_mask] = reward_tensor_gpt
#             acc_reward_tensor[~data_mask] = acc_reward_tensor_normal
#             acc_reward_tensor[data_mask] = acc_reward_tensor_gpt
#             format_reward_tensor[~data_mask] = format_reward_tensor_normal
#             format_reward_tensor[data_mask] = format_reward_tensor_gpt
#             overlong_reward_tensor[~data_mask] = overlong_reward_tensor_normal
#             overlong_reward_tensor[data_mask] = overlong_reward_tensor_gpt
#             invalid_uids = invalid_uids_normal + invalid_uids_gpt
#     else:
#         reward_tensor, acc_reward_tensor, format_reward_tensor, overlong_reward_tensor, invalid_uids = reward_fn(cur_batch)
...
class AsyncvLLMEngine:
    ...
    def _preprocess_prompt_to_async_rollout_requests(self, prompts: DataProto, n):
        ...
        for data_idx, _raw_prompt_id in enumerate(prompts.non_tensor_batch["raw_prompt_ids"]):
            ...
            # print(f"_video_path: {_video_path}")
