...
from .pyiam.sign import sign_rpc_request
...
class Client:
    def __init__(self, ak, sk, scene, caller='tiktok.aiic.data', idc='maliva', cluster='default'):
        ...
        idl_path = os.path.join(Path(__file__).parent.parent.parent, 'idl')

        self.smart_player = thriftpy2.load(os.path.join(idl_path, 'videoarch/smart_player.thrift'),
                                           module_name='smart_player_thrift')
        self.base = thriftpy2.load(os.path.join(idl_path, 'base.thrift'), module_name='base_thrift')
        self.client = euler.Client(self.smart_player.SmartPlayerService,
                                   f'sd://toutiao.videoarch.smart_player?idc={idc}&cluster={cluster}', timeout=10)
