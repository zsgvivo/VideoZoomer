...
def base64_to_pil(image_dict):
    """将 {'bytes': base64_str, 'path': str} 转换为 PIL.Image"""
    if not image_dict or not isinstance(image_dict, dict) or 'bytes' not in image_dict:
        raise ValueError("Input must be a dict with 'bytes' key")

    try:
        image_bytes = base64.b64decode(image_dict['bytes'])
...
class MultiModalDataset(Dataset):
    ...
    def process_video(self, video_id_or_path: str, video_fps: float, frames_upbound: int, max_pixels: int = 2048 * 2048, min_pixels: int = 512 * 512, storage_system: str = 'local', is_accurate: bool = True, preprocess=False):
        import cv2
        from PIL import Image
        import numpy as np
        import time
        ...
        try:
            ...
            if preprocess:
                cache_path = f'data/preprocessed_frames/fps{video_fps}_upbound{frames_upbound}_maxpixels{max_pixels}_minpixels{min_pixels}/{uuid.uuid5(uuid.NAMESPACE_DNS, video_id_or_path)}'
                def load_frame(i):
            ...
            return [0], [Image.open('data/blank_image.png')] # Blank frame fallback for video load errors
    def __len__(self):
        ...
        if is_multi_modal:  # expand image token
            ...
            if self.image_key == 'images':
                ...
                if self.pass_video_as_frames:
                    ...
                    raw_prompt = prompt_with_chat_template.replace('<video>', ''.join([f'<frame{i}_time{frame_time[i]:.2f}s><|vision_start|><|image_pad|><|vision_end|>' for i in range(len(frames))]))
                    prompt_with_chat_template = prompt_with_chat_template.replace('<video>', ''.join([f'<frame{i}_time{frame_time[i]:.2f}s><image>' for i in range(len(frames))]))
                    ...
                    if '<image>' in raw_prompt:
                        ...
                        row_dict['multi_modal_data']['image'] += [base64_to_pil(row_dict['image'])]
                        row_dict['prompt_img_num'] += 1
                    assert raw_prompt.count('<|vision_start|><|image_pad|><|vision_end|>') == len(row_dict['multi_modal_data']['image']), f"Number of images in prompt: {raw_prompt.count('<|vision_start|><|image_pad|><|vision_end|>')} does not match number of images in multi_modal_data: {len(row_dict['multi_modal_data']['image'])}"
                    image_inputs = self.processor.image_processor(row_dict['multi_modal_data']['image'], return_tensors='pt')
                    multimodal_input_grid_thw = image_grid_thw = image_inputs['image_grid_thw']
                    # row_dict['multi_modal_inputs'] = {key: val for key, val in image_inputs.items()}
                    place_holder_token = self.processor.image_token
                else:
                    raw_prompt = prompt_with_chat_template.replace('<video>', '<|vision_start|><|video_pad|><|vision_end|>')
...
if __name__ == '__main__':
    ...
    train_dataset = MultiModalDataset(
        data_path='data/VideoMMMU/qa_processed.json',
        tokenizer=tokenizer,
        processor=processor,
        prompt_key='prompt',
        image_key='videos',
    ...
    with ThreadPoolExecutor(max_workers=48) as executor:
        # 提交所有任务
        futures = [executor.submit(process_example, i) for i in range(len(train_dataset))]
        
        # 如果需要获取结果
        results = [future.result() for future in futures]
        print(example['multi_modal_data']['image'].shape)
