...
async def evaluate(video_path, problem, answer, extra_info, semaphore, client, model, rollout_num: int = 1, fps=0.2, max_pixels=100352, pass_video_as_frames: bool = True, frames_upbound=30, timeout=60):
    async with semaphore:  # 自动获取和释放信号量
        ...
        for _ in range(rollout_num):
            pred = await query_client_with_retry(client, model, messages, video_kwargs, rollout_num=1,timeout=timeout)
            preds.extend(pred)
...
async def main():
    ...
    for problem_id, item in enumerate(train_set[:]):
        ...
        task = asyncio.create_task(evaluate(video_url, item['problem'], str(item['solution']), item['extra_info'],semaphore ,client ,args.model, args.eval_rollout_num, args.fps, args.max_pixels, pass_video_as_frames=args.pass_video_as_frames, frames_upbound=args.frames_upbound, timeout=args.request_timeout))
        tasks.append(task)
    results = await tqdm_asyncio.gather(*tasks)
    for idx, result in enumerate(results):
        train_set[idx]['pass_rate'] = result['pass_rate']
        counter[result['pass_rate']] += 1
        train_set[idx]['detailed_rollout'] = result['detailed_rollout']
