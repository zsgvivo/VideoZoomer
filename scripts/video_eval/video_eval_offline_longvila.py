...
def judge_multi_choice(prompt_str: str, predict_str: str, ground_truth: str) -> dict:
    ...
    predict_str = sol_match.group(1).strip() if sol_match else predict_str.strip()
...
def process_video(video_id_or_path: str, max_pixels: int = 100352, min_pixels: int = 25088, storage_system: str = 'local', is_accurate: bool = True, preprocess=True, video_fps=0.5, frames_upbound=64, is_fix_frame=False):
    from io import BytesIO
    import cv2
    from PIL import Image
    import numpy as np
    ...
    try:
        ...
        if preprocess:
            fps_str = str(int(video_fps)) if video_fps.is_integer() else str(video_fps)
            cache_path = f'/mnt/bn/tiktok-mm-5/aiic/users/dingyang/data/preprocessed_frames/fps{fps_str}_upbound{frames_upbound}_maxpixels{max_pixels}_minpixels{min_pixels}/{uuid.uuid5(uuid.NAMESPACE_DNS, video_id_or_path)}'
            ...
            try:
                if os.path.exists(cache_path):
                    ...
                    if len(frame_time) %4 !=0 :
                        raise ValueError(f'frame num {len(frame_time)} is not divisible by 4, re-processing')
                    with ThreadPoolExecutor(max_workers=8) as executor:
                        frames = list(executor.map(load_frame, range(len(frame_time))))
                    logger.debug(f'video {video_id_or_path} is preprocessed, loaded in {time.time() - start_time:.2f}s')
                ...
                logger.warning(f'Exception: {exn}')
        ...
        if frames_upbound > 0:
            if is_fix_frame or len(frame_idx) > frames_upbound:
        ...
        while len(pil_images) % 4 != 0:  # LongVila encoder 需要确保帧数是4的倍数
            pil_images.append(pil_images[-1])
            frame_time.append(frame_time[-1])
        logger.debug(f"total_frame_num:, {total_frame_num}, video_time:, {video_time}, avg_fps:, {avg_fps}, frame num:, {len(pil_images)}, 'processing time:', {time.time() - start_time}")
        del vr
        ...
        logger.error(f"Failed to process video {video_id_or_path}. Exception:{exn}")
...
def query_client_with_retry(client, model, messages, semaphore=None):
    ...
    response_str = chat_response.choices[0].message.content
    return response_str
def build_message(video_path, problem, answer, extra_info, rollout_num: int = 1, fps=0.2, max_pixels=100352, frames_upbound=64,encoder_device=0):
    _, pil_images = process_video(video_path, max_pixels=max_pixels, video_fps=fps, frames_upbound=frames_upbound)
    ...
    try:
        user_prompt = system_prompt_thinking.format(question=problem.replace('<image>', '<vila/video>'))
        # conversation = [{"from": "human", "value": user_prompt}]
        conversation = [{"from": "human", "value": user_prompt}]
        media = defaultdict(list)
        media["video"] = [image_tensors]
        input_ids = tokenize_conversation(
            conversation, model_encoder.tokenizer, add_generation_prompt=True
        ).unsqueeze(0).cuda(encoder_device)
        ...
        if 'CUDA' in str(e):
            raise e
        logger.error(f"Failed to embed conversation: {e}")
        logger.debug(f"{media['video'][0].shape}")
        user_prompt = system_prompt_thinking.format(question=problem.replace('<image>', ''))
        conversation = [{"from": "human", "value": user_prompt}]
        media = defaultdict(list)
        input_ids = tokenize_conversation(
            conversation, model_encoder.tokenizer, add_generation_prompt=True
    ...
    logger.debug(f"input_ids: {input_ids.shape}; inputs_embeds: {inputs_embeds.shape}")
    return {"prompt_embeds": inputs_embeds.squeeze(0)}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='/mnt/hdfs/yangding2/models/LongVILA-R1-7B',
                        help='model name to eval')
    parser.add_argument('--batch_size', type=int, default=20,
                        help='Maximum number of concurrent tasks')
    parser.add_argument('--tp_size', type=int, default=1)
    parser.add_argument('--frames_upbound', type=int, default=64)
    parser.add_argument('--encoder_device', type=int, default=1)
    ...
    ).cuda(args.encoder_device)
    
    # tokenizer = AutoTokenizer.from_pretrained(args.model, trust_remote_code=True)
    # llm = LLM(args.model, limit_mm_per_prompt={"image": 20}, tensor_parallel_size=4)
    llm = LLM(
        model=os.path.join(args.model, "llm"),
        enable_prompt_embeds=True,
        gpu_memory_utilization=0.5,
        tensor_parallel_size=args.tp_size
    )

    device = next(model_encoder.parameters()).device   # 一般是 cuda:0
    ...
    # 初始化队列和线程
    video_processing_queue = Queue(maxsize=3)  # 控制内存使用
    results_queue = Queue()
    
    # 创建线程池用于视频处理
    video_executor = concurrent.futures.ThreadPoolExecutor(max_workers=args.max_workers)
    ...
    def video_processor():
        for i in range(0, len(train_set), batch_size):
            ...
            # 提交视频处理任务到线程池
            futures = [video_executor.submit(process_item, item) for item in batch]
            # 等待所有视频处理完成
            prompt_embeds_batch = [f.result() for f in futures]
            video_processing_queue.put((i, batch, prompt_embeds_batch))
        video_processing_queue.put(None)  # 结束信号
    
    # 消费者线程 - 模型推理
    def model_inferencer():
        while True:
            item = video_processing_queue.get()
            if item is None:
                break
            i, batch, prompt_embeds_batch = item
            # GPU 处理
            completions = llm.generate(
                prompts=prompt_embeds_batch,
            ...
            # assert i == results_queue.qsize()
            logger.debug(f'sample {i} processed')
        results_queue.put(None)
    
    # 启动线程
    video_thread = Thread(target=video_processor)
    inferencer_thread = Thread(target=model_inferencer)
    ...
    save_path = os.path.join(save_dir, f'{base_name}_model{model_name}_eval_rollout{args.eval_rollout_num}_fps{args.fps}_max_pixels{args.max_pixels}_{now}.json')
    with open(save_path, 'w') as f:
        json.dump(train_set, f, indent=4)
    logger.info(f'results saved to {save_path}')
    # save pass rate
    save_dir2 = os.path.join(save_dir, f'{base_name}_model{model_name}_eval_rollout{args.eval_rollout_num}_fps{args.fps}_max_pixels{args.max_pixels}_passrate_{now}.json')
