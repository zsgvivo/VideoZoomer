        ...
        if pass_video_as_frames:
            ...
            if use_fewshots:
                ...
                for fewshot_idx in [6,7]:
                    ...
                    idx += 1
                    ex_msg, imgs = load_example(f"data/ytb_json_files/sft/examples/{fewshot_idx}.json")
        ...
        for i in range(rollout_num):
            try:
                ...
                if save_sft:
                    ...
                    sft_res = extract_message_to_sft_format(new_messages, f"data/sft_images/{now}/{unique_id}", prompt_image_num=(len(prompt_images) if use_fewshots else 0), prompt_image_paths=prompt_images)
                    res['sft_messages'] = {
                        "messages": sft_res[0],
                        "images": sft_res[1]
                    }
...
async def main():
    ...
    # 添加命令行参数
    parser.add_argument('--json_path', type=str, default='scripts/video_eval/test.json',
